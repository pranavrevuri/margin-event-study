#!/usr/bin/env python3
"""
Preregistration v2.0 — PART B, Stage 1 ONLY.
...
the Stage-1 events_v2.csv schema and A4; nothing after entry is computed.)
"""
import math
import random
import collections
...
def contract_price_series(mp):
    ...
    PRICE/FORWARD/CARRY appearances and ALL intraday rows (last non-NaN of day)"""
...
# ---------------- contract selection & window completeness (A4) ----------------
rows = []
hg_may_count = 0
for ev in sorted(events, key=lambda e: (e["product"], e["effective_date"])):
    ...
    rec = dict(product=prod, effective_date=d0, entry_date=entry, selected_contract="",
               ...
               cluster_week="", included_flag=False, exclusion_reason="")
    ...
    # roll within 11 trading days after entry -> FORWARD
    end11 = td_shift(prod, entry, 11)
    rcp = rolls[prod]
    roll_soon = ((rcp.date > entry) & (rcp.date <= (end11 or "9999"))).any()
    if roll_soon:
        if prow.FORWARD_CONTRACT == prow.FORWARD_CONTRACT:
            sel = str(int(prow.FORWARD_CONTRACT))[:6]
        else:
...
ev_df.to_csv(DATA / "events_v2.csv", index=False)

# ---------------- hygiene (Stage 1 step 3) ----------------
print("=" * 78)
print("STAGE 1 HYGIENE [analysis span: per-product binding start -> 2024-03-28]")
...
si_first = min(min(s) for s in [list(cseries["SI"].values())[0].keys()] ) if cseries["SI"] else "9999"
...
# (c) window holes: all exclusions are explicit in events_v2.csv
...
# ---------------- K0 gates (A8) ----------------
print("=" * 78)
print("K0 GATES [6 confirmatory products; spans: binding start -> 2024-03-28]")
...
overall_survival = ev_df.included_flag.mean()
dropped = [p for p in per_prod.index if per_prod.loc[p, "survival"] < 0.70]
if not si_resolved and "SI" not in dropped:
    dropped.append("SI")
remaining = [p for p in per_prod.index if p not in dropped]
...
print(f"\nGATE 1  total qualifying clustered events (full span): {total_events}  vs >= 180  -> {'PASS' if g1 else 'FAIL'}")
print(f"GATE 2  pilot qualifying clustered events (binding start -> 2014-12-31): {n_pilot}  vs >= 50  -> {'PASS' if g2 else 'FAIL'}")
print(f"GATE 3  window-completeness survival: {overall_survival*100:.1f}% overall vs >= 90%;"
